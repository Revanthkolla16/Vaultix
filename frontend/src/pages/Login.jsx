...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null)
    try {
      const res = await fetch('https://vaultix-qs63.onrender.com/api/auth/login', {
        method: 'POST',
      ...
      <main className="flex-1 flex items-center justify-center py-30">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 w-full max-w-md shadow-xl hover:shadow-2xl hover:shadow-purple-500/10 hover:scale-105 transition-transform duration-300">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Login to Vaultix</h1>
          ...
          <div className="text-center mt-6">
            <Link to="/register" className="text-purple-400 hover:text-pink-400 transition font-medium">Don't have an account? Sign Up</Link>
          </div>
        </div>
      </main>
      <Footer />
